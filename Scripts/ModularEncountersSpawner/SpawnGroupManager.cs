	...
	public static class SpawnGroupManager{
		...
		public static void CreateSpawnLists(){
			...
			foreach(var spawnGroup in regularSpawnGroups){
				...
				if(spawnGroup.Enabled == false || SpawnResources.CheckDefinitionProperties(spawnGroup) == false){
				...
				if(tag.Contains("[FactionOwner") == true){
                    ...
                    if(improveSpawnGroup.FactionOwner.ToUpper().Contains(Encoding.UTF8.GetString(Convert.FromBase64String("UElSQVRJWA=="))) == true) {
                        ...
                        if(string.IsNullOrWhiteSpace(spawnGroup.Context?.ModId) == false) {
                            ...
                            SpawnResources.CheckDefinitionProperties(spawnGroup, spawnGroup.Context.ModId);
